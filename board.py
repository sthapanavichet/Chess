...
class Board:
    ...
    game_position = {i * 10 + j: 0 for i in range(1, 9) for j in range(1, 9)}
    game_position.update({
        51: 1, 41: 2, 11: 3, 81: 3, 31: 4, 61: 4, 21: 5, 71: 5,
        12: 6, 22: 6, 32: 6, 42: 6, 52: 6, 62: 6, 72: 6, 82: 6
    })
    game_position.update({
    ...
    def __init__(self, white_position, black_position, game_position, castling_flags=None, en_passant_flag=0, move_counter=0):
        self.white_position = white_position.copy()
        self.black_position = black_position.copy()
        self.game_position = game_position.copy()
    ...
    def check_select(self, position):
        if self.move_counter % 2 == 0:
            return self.white_position[position] != 0
        else:
            return self.black_position[position] != 0

    # get the square that is clicked on


    def copy(self):
        return Board(self.white_position, self.black_position, self.game_position, self.castling_flags, self.en_passant_flag, self.move_counter)

    def make_move(self, position, move):
        piece = self.game_position[position]
        ...
        if self.move_counter % 2 == 0:
            ...
            if move // 100 == 0:
                ...
                move %= 200
                self.game_position[move-1] = 0
                self.black_position[move-1] = 0
            else:
                if move == 331:
                    move = 31
                    self.game_position[41] = 3
                    self.game_position[11] = 0
                    self.white_position[41] = 3
                    self.white_position[11] = 0
                else:
                    move = 71
                    self.game_position[61] = 3
                    self.game_position[81] = 0
                    self.white_position[61] = 3
                    self.white_position[81] = 0
            self.white_position[move] = piece
            self.white_position[position] = 0
            self.black_position[move] = 0
        else:
            if piece == 1:
            ...
            if move // 100 == 0:
                ...
                move %= 200
                self.game_position[move + 1] = 0
                self.white_position[move + 1] = 0
            else:
                if move == 338:
                    move = 38
                    self.game_position[48] = 3
                    self.game_position[18] = 0
                    self.black_position[48] = 3
                    self.black_position[18] = 0

                else:
                    move = 78
                    self.game_position[68] = 3
                    self.game_position[88] = 0
                    self.black_position[68] = 3
                    self.black_position[88] = 0
            self.black_position[move] = piece
            self.black_position[position] = 0
            self.white_position[move] = 0
        self.game_position[move] = piece
        self.game_position[position] = 0
        self.move_counter += 1
    ...
    def pawn_move(self, position):
        ...
        if self.move_counter % 2 == 0:
            ...
            for pawn_threat in pawn_threats:  # check for valid moves in the possible moves
                ...
                if self.black_position[pawn_threat] != 0:
                    available_moves.append(pawn_threat)
            if row == 2:
                for i in range(1, 3):
                    move = col * 10 + row + i
                    if self.game_position[move] != 0:
                        break
            ...
            for pawn_threat in pawn_threats:  # check for valid moves in the possible moves
                # add threats to available moves if there is an enemy piece on the position
                if self.white_position[pawn_threat] != 0:
                    available_moves.append(pawn_threat)
            if row == 7:
                for i in range(1, 3):
                    move = col * 10 + row - i
        ...
        if self.move_counter % 2 == 0:  # white king
            ...
            if self.castling_flags[1]:
                if self.castling_flags[11]:  # right side castling
                    if 61 in available_moves and self.game_position[71] == 0:
                        if 71 not in black_threats:
                            available_moves.append(371)
                if self.castling_flags[12]:  # left side castling
                    if self.game_position[21] == 0 and self.game_position[31] == 0 and 41 in available_moves:
            ...
            if self.castling_flags[2]:
                if self.castling_flags[21]:  # right side castling
                    if 68 in available_moves and self.game_position[78] == 0:
                        if 78 not in white_threats:
                            available_moves.append(378)
                if self.castling_flags[22]:  # left side castling
                    if self.game_position[28] == 0 and self.game_position[38] == 0 and 48 in available_moves:
    ...
    def in_check(self):
        if self.move_counter % 2 == 0:
            ...
            for black_threat in black_threats:
                if self.white_position[black_threat] == 1:
                    return black_threat
        else:
            white_threats = self.get_white_threats()
            for white_threat in white_threats:
                if self.black_position[white_threat] == 1:
                    return white_threat
        return None
    ...
    def available_move(self, position):
        ...
        for move in possible_moves:
            game_copy = self.copy()
            game_copy.try_move(position, move)
            if game_copy.in_check() is None:
                available_moves.append(move)
        return available_moves
...
if __name__ == '__main__':
    ...
    board = Board(white_position, black_position, game_position)
    # print(board.minimax(4, 1))
    print(board.generate_position(5, 1))
    print(timer() - start)
    pygame.quit()
    sys.exit()
    # main()